    ...
    public void handle(ActionEvent btn_load){
        ...
        if(btn_load.getSource().equals(AIManager.btn_reset)){
            ...
            ac_text.setText("Aircraft Number");
                ...
                System.out.println(ac_array.get(7).getHasLongGround());
        ...
        if(btn_load.getSource().equals(AIManager.btn_search)){
            ...
            for(int j = 0; j < ac_array.size(); j++){
                ...
                if ((ac_array.get(j).getHasLongGround() && ac_array.get(j).getFlightnum().contains(flight_num) && String.valueOf(ac_array.get(j).getAcnum()).contains(ac_num)) ||
                   ...
                   (ac_array.get(j).getHasLongGround() && flight_num.equals("*") && ac_num.equals("*"))){
                    ...
                    Line h_lines = new Line();
                    h_lines.setStartX(0);
                    h_lines.setStartY(((nm_results-1)*30)+18);
                    h_lines.setEndX(2000);
                    h_lines.setEndY((nm_results-1)*30+18);
                    h_lines.setStroke(Color.LIGHTSLATEGREY);

                    //AIRCRAFT LABELS
                    Label ac_label = new Label((nm_results-1)+1+" AC#"+String.valueOf(ac_array.get(j).getAcnum())+" "+ac_array.get(j).getFlightnum());
                    ac_label.setLayoutX(0);
                    ac_label.setLayoutY((nm_results-1)*30+20);
                    ac_label.setTextFill(Color.BLACK);
                    ...
                    for(int i = 0; i < ac_array.get(j).flight_array.size(); i++){
                        ...
                        //Drawing Aircrafts
                        //REC(X START, Y START, WIDTH, HEIGHT)
                        Rectangle leg_rec = new Rectangle(dep_time_in_gui+30,(nm_results-1)*30+26,duration_in_gui,15);
                        Tooltip stations = new Tooltip();
                        stations.setText(ac_array.get(j).getFlight(i).getDepstation()+" - "+ac_array.get(j).getFlight(i).getArrstation()+"\n"+
                                         ac_array.get(j).getFlight(i).getDeptime()+" - "+ac_array.get(j).getFlight(i).getArrtime());
                        if (ac_array.get(j).getFlight(i).getDeptime() >= 2400){
            ...
            AIManager.ac_count_txt.setText("Aircraft Loaded: "+nm_results);
        }
